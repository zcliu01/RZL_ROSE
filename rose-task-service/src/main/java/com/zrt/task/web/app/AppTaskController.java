...
import com.zrt.task.web.manage.TaskController;

@RestController
@RequestMapping("/v1.0/apptask")
public class AppTaskController {
	...
	public Map<String, Object> callChild(@RequestBody Map<String, Object> map) throws Exception {
		...
		try {
			// 新建任务
			int result = taskService.callChild(map);
			if (result==2) {
				return resultMap.failure().errmsg("分配任务失败(未识别父任务id)!").map();
			}
	...
	public Map<String, Object> search(@RequestBody Map<String, Object> map) throws Exception {
		log.info("Server端的TaskController中search方法");
		ResultMap resultMap = new ResultMap();
		try {
			Map<String, Object> search = taskService.search(map);
			resultMap.success().map().put("count", search.get("count"));
			resultMap.data(search.get("data"));
			resultMap.success().sucmsg("任务列表查询成功!");
			return resultMap.map();
		} catch (AppException e) {
			log.error("json error", e);
			return resultMap.failure().errmsg(e.getMessage()).map();
			...
			log.error("json error", e);
			return resultMap.failure().errmsg("任务列表查询失败!").map();
	...
	public Map<String, Object> stop(@RequestBody Map<String, Object> map) throws Exception {
		...
		try {
			...
			log.error("json error", e);
			return resultMap.failure().errmsg("任务终止失败!").map();
