package com.zrt.flow.web.manage.flowself;
...
@RestController
@RequestMapping("/v1.0/flowself")
public class FlowselfController {
	private static final Logger log = LoggerFactory.getLogger(FlowselfController.class);
	...
	public Map<String, Object> flowList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中selfApply方法");
	...
	public Map<String, Object> selfDoList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中selfDoList方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 格式化查询时间
			if (StringUtils.isNotBlank((String) appMap.get("begin_start_date_time"))) {
	...
	public Map<String, Object> selfSign(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中selfSign方法");
		ResultMap resultMap = new ResultMap();
		try {
	...
	public Map<String, Object> check(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中check方法");
		ResultMap resultMap = new ResultMap();
		try {
	...
	public Map<String, Object> selfHasDoList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中selfHasDoList方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 格式化查询时间
			if (StringUtils.isNotBlank((String) appMap.get("begin_start_date_time"))) {
	...
	public Map<String, Object> selfApplyRes(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中selfApplyRes方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 格式化查询时间
			if (StringUtils.isNotBlank((String) appMap.get("begin_start_date_time"))) {
			...
			log.error(" error", e);
			return resultMap.failure().errmsg("我的待回复查询失败!").map();
		}
	}
	 ...
	 * @throws AppException
	 */
	@RequestMapping(value = "/selfApplyConfirm", method = RequestMethod.POST, headers = "Accept=application/json")
	public Map<String, Object> selfApplyConfirm(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的FlowselfController中selfApplyConfirm方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 格式化查询时间
			if (StringUtils.isNotBlank((String) appMap.get("begin_start_date_time"))) {
			...
			Map<String, Object> resMap = flowselfService.selfApplyConfirm(appMap);
			resultMap.data(resMap.get("data"));
			resultMap.success().map().put("count", resMap.get("count"));
			return resultMap.map();
		} catch (AppException e) {
