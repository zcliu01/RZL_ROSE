package com.zrt.flow.web.manage.handle;
...
public class HandleController {
	...
	public Map<String, Object> draftDetail(@RequestBody Map<String, Object> appMap) throws Exception{
		log.info("Server端的HandleController中draftDetail方法");
		ResultMap resultMap=new ResultMap();
		try {
			 Map<String, Object> resData=handleService.draftDetail(appMap);
			resultMap.success().data(resData);
			return resultMap.map();
		}catch(AppException e){
			log.error(" error", e);
	 ...
	 * @throws AppException
	 */
    @RequestMapping(value="/instInfor", method = RequestMethod.POST, headers = "Accept=application/json")
	public Map<String, Object> instInfor(@RequestBody Map<String, Object> appMap) throws Exception{
		log.info("Server端的HandleController中instInfor方法");
		ResultMap resultMap=new ResultMap();
		try {
			 Map<String, Object> resData=handleService.instInfor(appMap);
			resultMap.success().data(resData);
			return resultMap.map();
		}catch(AppException e){
			log.error(" error", e);
			...
			log.error(" error", e);
			return resultMap.failure().errmsg("流程查询失败!").map();
		}
	}
