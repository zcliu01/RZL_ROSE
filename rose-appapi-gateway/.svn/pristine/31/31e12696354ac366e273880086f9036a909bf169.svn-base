...
@RestController
@RequestMapping(value="/v1.0/appdaywork", produces = { "application/json;charset=UTF-8" })
public class DayWorkController extends BaseController {
	 ...
	 * 查询所有日期
	 */
	@RequestMapping(value = "/searchDate",method = RequestMethod.POST,consumes=ApiConstant.CONSUMES,headers = ApiConstant.HEADERS)
	@ResponseBody   
	public ResponseEntity<String> searchDate(@RequestBody Map<String, Object> requestMap,@CookieValue String emp_id) throws Exception {
		log.info("Server端的DayWorkController中searchDate方法");
		setEmpId(emp_id, requestMap);
		Map<String,Object>  resultMap = dayWorkClient.searchDate(requestMap);
		return response(resultMap);
	}
	 ...
	 * 批量删除
	 */
	@RequestMapping(value = "/deleteList",method = RequestMethod.POST,consumes=ApiConstant.CONSUMES,headers = ApiConstant.HEADERS)
	@ResponseBody   
	public ResponseEntity<String> deleteList(@RequestBody Map<String, Object> requestMap,@CookieValue String emp_id) throws Exception {
		log.info("Server端的DayWorkController中deleteList方法");
		setEmpId(emp_id, requestMap);
		Map<String,Object>  resultMap = dayWorkClient.deleteList(requestMap);
		return response(resultMap);
	}
}
