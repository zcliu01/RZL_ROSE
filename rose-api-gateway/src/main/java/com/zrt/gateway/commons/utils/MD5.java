...
public class MD5 {
 ...
 public static byte[] encodeVpn(String pw) {
	 try {  
		 ...
		 byte[] resultByteArray = messageDigest.digest();  
		 // 字符数组转换成字符串返回  
		 byte[] newResultByteArray=new byte[24];
		System.arraycopy(resultByteArray, 0, newResultByteArray, 0, 16);
		System.arraycopy(resultByteArray, 0, newResultByteArray, 16, 8);
		 return newResultByteArray;  
	 } catch (NoSuchAlgorithmException e) {  
		 return null;  
	 }  
 }
