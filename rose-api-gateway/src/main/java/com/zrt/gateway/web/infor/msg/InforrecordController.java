...
public class InforrecordController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/selectReceiver", method = RequestMethod.POST, consumes = ApiConstant.CONSUMES, headers = ApiConstant.HEADERS)
	@ResponseBody
	public ResponseEntity<String> saveAndSendInfor(@RequestBody Map<String, Object> requestMap) throws Exception {
		log.info("Server端的InforrecordController中selectReceiver方法");
		Map<String, Object> resultMap = inforClient.selectReceiver(requestMap);
		return response(resultMap);
	}
}
