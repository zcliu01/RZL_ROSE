...
@RestController
@RequestMapping(value = "/v1.0/flowself", produces = { "application/json;charset=UTF-8" })
public class FlowSelfController extends BaseController {
	...
	@ResponseBody
	public ResponseEntity<String> selfHashDoList(@RequestBody Map<String, Object> requestMap,
			@CookieValue String emp_id) throws Exception {
		log.info("Server端的AppFlowSelfControl中selfHashDoList方法");
		setEmpId(emp_id, requestMap);
		Map<String, Object> resultMap = flowClient.selfHashDoList(requestMap);
		return response(resultMap);
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/selfApplyConfirm", method = RequestMethod.POST, consumes = ApiConstant.CONSUMES, headers = ApiConstant.HEADERS)
	@ResponseBody
	public ResponseEntity<String> selfApplyConfirm(@RequestBody Map<String, Object> requestMap,
			@CookieValue String emp_id) throws Exception {
		log.info("Server端的AppFlowSelfControl中selfApplyConfirm方法");
		setEmpId(emp_id, requestMap);
		Map<String, Object> resultMap = flowClient.selfApplyConfirm(requestMap);
		return response(resultMap);
	}
