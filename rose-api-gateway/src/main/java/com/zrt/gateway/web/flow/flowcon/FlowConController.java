...
@RestController
@RequestMapping(value = "/v1.0/con", produces = { "application/json;charset=UTF-8" })
public class FlowConController extends BaseController {
	private static final Logger log = LoggerFactory.getLogger(FlowConController.class);
	...
	public ResponseEntity<String> res(@RequestBody Map<String, Object> requestMap, @CookieValue String emp_id)
			throws Exception {
		log.info("Server端的FlowConController中res方法");
	...
	public ResponseEntity<String> confirm(@RequestBody Map<String, Object> requestMap, @CookieValue String emp_id)
			throws Exception {
		log.info("Server端的FlowConController中confirm方法");
		setEmpId(emp_id, requestMap);
