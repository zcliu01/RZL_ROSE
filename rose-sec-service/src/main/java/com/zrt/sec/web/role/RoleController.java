...
public class RoleController {
	...
	public Map<String, Object> roleList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleList方法");
		...
		try {
			...
			// 获取用户角色列表
			appMap.put("role_type", "0");// 角色类型；0：系统权限角色
	...
	public Map<String, Object> roleSearch(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleSearch方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 获取用户角色信息
	...
	public Map<String, Object> roleUserList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中userRoleList方法");
		ResultMap resultMap = new ResultMap();
		try {
			...
			if (appMap.get("f_join_start_time") != null && !"".equals(appMap.get("f_join_start_time"))) {
				appMap.put("f_join_start_time", appMap.get("f_join_start_time").toString().replaceAll("-", "") + "000000");
			}
	...
	public Map<String, Object> roleEmpDelete(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleEmpDelete方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 用户角色删除
	 ...
	 * 角色一级菜单查询
	 */
	@RequestMapping(value = "/roleMenuList", method = RequestMethod.POST, headers = "Accept=application/json")
	public Map<String, Object> roleMenuList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleMenuList方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 角色一级菜单查询
			Map<String, Object> resMap = roleService.roleMenuList(appMap);
			...
			log.error("json error", e);
			return resultMap.failure().errmsg("一级菜单查询失败!").map();
		}
	}
	 ...
	 */
	/*
	 * @RequestMapping(value="/roleResourceSearch", method = RequestMethod.POST,
	 * headers = "Accept=application/json") public Map<String, Object>
	 * roleResourceSearch(@RequestBody Map<String, Object> appMap) throws
	 * Exception{ log.info("Server端的RoleController中roleResourceSearch方法");
	 * ResultMap resultMap=new ResultMap(); try { //角色一级菜单查询 List<Map<String,
	 * Object>> resMap=roleService.roleResourceSearch(appMap); Map<String,
	 * Object> map=new HashMap<String, Object>(); map.put("role_id",
	 * appMap.get("role_id"));//角色id map.put("root_menu_code",
	 * appMap.get("root_menu_code"));//根菜单编号 map.put("resourceList", resMap);
	 * resultMap.success(); resultMap.data(map); return resultMap.map(); } catch
	 * (AppException e) { log.error("json error", e); return
	 * resultMap.failure().errmsg(e.getMessage()).map(); }catch(Exception e){
	 * log.error("json error", e); return
	 * resultMap.failure().errmsg("一级菜单查询失败!").map(); } }
	 */
	@RequestMapping(value = "/roleResourceSearch", method = RequestMethod.POST, headers = "Accept=application/json")
	public Map<String, Object> roleResourceSearch(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleResourceSearch方法");
		ResultMap resultMap = new ResultMap();
		try {
			List<Map<String, Object>> resourceList = roleService.roleResourceSearch(appMap);
			resultMap.success().data(resourceList);
			return resultMap.map();
		} catch (AppException e) {
			log.error("json error", e);
			return resultMap.failure().errmsg(e.getMessage()).map();
			...
			log.error("json error", e);
			return resultMap.failure().errmsg("一级菜单下的资源查询失败!").map();
		}
	}
	 ...
	 * 角色资源绑定
	 */
	@RequestMapping(value = "/roleResourceSet", method = RequestMethod.POST, headers = "Accept=application/json")
	public Map<String, Object> roleResourceSet(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleResourceSet方法");
		ResultMap resultMap = new ResultMap();
		try {
			// 角色功能绑定
			roleService.roleResourceSet(appMap);
			resultMap.success().sucmsg("绑定成功!");
			return resultMap.map();
		} catch (AppException e) {
			log.error("json error", e);
			...
			log.error("json error", e);
			return resultMap.failure().errmsg("角色功能绑定失败!").map();
		}
	}
	...
	public Map<String, Object> roleAllList(@RequestBody Map<String, Object> appMap) throws Exception {
		log.info("Server端的RoleController中roleAllList方法");
		ResultMap resultMap = new ResultMap();
		try {
