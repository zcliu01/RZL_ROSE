...
public class CipherUtil {
	...
	// vpn加密3des
	public static final String VPN_TYPE = "TripleDES";
	// vpn加密key
	public static final String VPN_KEy = "zrt12345";

	/**
	 * vpn解密
	 */
	public static String decryptVpn(String target) {

		try {

			byte[] targetToByte = decryptByBase64(target); // 需要BASE64包装一下，否则会抛出异常
			Cipher cipher = Cipher.getInstance(VPN_TYPE);
			// System.out.println(MD5.byteArrayToHex(MD5.encodeVpn(VPN_KEy)));
			cipher.init(Cipher.DECRYPT_MODE, getVpnKey(MD5.encodeVpn(VPN_KEy), VPN_TYPE));
	 ...
	 * vpn解密Plus
	 */
	public static String decryptMode(String target) {

		try {

			SecretKey deskey = new SecretKeySpec(MD5.encodeVpn(VPN_KEy), VPN_TYPE);

			Cipher c1 = Cipher.getInstance(VPN_TYPE);

			c1.init(Cipher.DECRYPT_MODE, deskey);

			return new String(c1.doFinal(Base64.decode(target)), "UTF-8");
...
//		System.out.println(decryptMode("Sd9nDPibuTQ="));
//	}

	/**
	 * vpn加密
	 */
	public static String encryptVpn(String target) {

		try {

			byte[] targetToByte = target.getBytes("UTF-8");// 需要BASE64包装一下，否则会抛出异常
			Cipher cipher = Cipher.getInstance(SEC_TYPE);
			System.out.println(MD5.byteArrayToHex(MD5.encodeVpn(VPN_KEy)));
			cipher.init(Cipher.ENCRYPT_MODE, getVpnKey(MD5.encodeVpn(VPN_KEy), SEC_TYPE));
			byte[] result = cipher.doFinal(targetToByte);
	...
	public static Key getVpnKey(byte[] key, String algorithm) {
		try {
	...
	/**
	 * public static void main(String[] args) { //String target = "测试"; //
	 * String target = "test"; // DES // String encrypt = encrypt(target, "DES",
	 * "im a ky" ); // System.out.println("加密后：" + encrypt); // String decrypt
	 * =decrypt(encrypt, "DES", "im a ky"); // System.out.println("解密后：" +
	 * decrypt); // 3DES // Security.addProvider(new
	 * com.sun.crypto.provider.SunJCE()); String encrypt1 = encryptVpn(target);
	 * System.out.println("加密后：" +encrypt1); String decrypt1 =
	 * decryptVpn(encrypt1); System.out.println("解密后：" + decrypt1); // AES
	 * 
	 * // String encrypt2 = encrypt(target, "AES", "im a key"); //
	 * System.out.println("加密后：" + encrypt2); // String decrypt2
	 * =decrypt(encrypt2, "AES", "im a key"); // System.out.println("解密后："
	 * +decrypt2); }
	 **/
