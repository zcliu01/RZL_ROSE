package com.zrt.infor.web.manage.inforControl;
...
@RestController
@RequestMapping("/v1.0/inforControl")
public class InforControlController {
	private static final Logger log = LoggerFactory.getLogger(InforControlController.class);
	...
	public Map<String, Object> withInforDetails(@RequestBody Map<String, Object> map) throws Exception {
		...
		try {
			...
			Map<String, Object> resMap = inforControlService.getControlDetails(map);
			resultMap.success().map().put("count", resMap.get("count"));
			resultMap.data(resMap.get("data"));
			return resultMap.map();
		} catch (AppException e) {
	...
	public Map<String, Object> inforDetailsSearch(@RequestBody Map<String, Object> map) throws Exception {
		...
		try {
			...
			Map<String, Object> resMap = inforControlService.getDetailsSearch(map);
			resultMap.success().map().put("count", resMap.get("count"));
			resultMap.data(resMap.get("data"));
			return resultMap.map();
		} catch (AppException e) {
	...
	public Map<String, Object> withEmpList(@RequestBody Map<String, Object> map) throws Exception {
		...
		try {
			...
			Map<String, Object> resMap = inforControlService.getWithEmpList(map);
			resultMap.success().map().put("count", resMap.get("count"));
			resultMap.data(resMap.get("data"));
			return resultMap.map();
		} catch (AppException e) {
